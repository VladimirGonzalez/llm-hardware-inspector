...
def run_cli():
    parser = argparse.ArgumentParser(
        ...
        description="Herramienta CLI para inspeccionar hardware y sugerir configuración de LLM."
    )
    parser.add_argument(
        "--output",
        choices=["text", "json"],
        default="text",
        help="Formato de salida: texto por defecto, o JSON para parsear programáticamente."
    )
    parser.add_argument(
        "--output-file",
        type=str,
        help="Si se especifica, la información (texto/JSON) se guardará en este archivo."
    )
    parser.add_argument(
        "--prompt-file",
        type=str,
        default="sugerencias_llm.txt",
        help="Nombre del archivo donde se guardarán las sugerencias (texto)."
    )
