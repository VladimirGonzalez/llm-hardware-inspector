...
def obtener_info_sistema() -> dict:
    ...
    if cpu_freq:
        info["Frecuencia Móxima (MHz)"] = cpu_freq.max
        info["Frecuencia Mónima (MHz)"] = cpu_freq.min
        info["Frecuencia Actual (MHz)"] = cpu_freq.current
    else:
        info["Frecuencia Móxima (MHz)"] = None
        info["Frecuencia Mónima (MHz)"] = None
        info["Frecuencia Actual (MHz)"] = None

    info["Nócleos Fósicos"] = psutil.cpu_count(logical=False)
    info["Nócleos Totales (lógicos)"] = psutil.cpu_count(logical=True)
    info["Uso de CPU por Nócleo (%)"] = psutil.cpu_percent(percpu=True)
    ...
    try:
        ...
        for gpu in gpus:
            gpu_info = {
                ...
                "Temperatura (óC)": gpu.temperature
            }
            gpu_data.append(gpu_info)
    except Exception as e:
        gpu_data.append({"Error": f"No se pudo obtener información de la GPU vóa GPUtil: {e}"})
...
def generar_prompt_sugerencias(info: dict) -> str:
    ...
    if total_vram_gb >= 24:
        ...
        sugerencias.append("Podrías usar modelos 7B con tócnicas de cuantización (4 bits/8 bits).")
    elif total_vram_gb > 0:
        sugerencias.append("Quizó sólo modelos muy pequeóos o fuertemente cuantizados en GPU.")
    else:
        sugerencias.append("No se detectó una GPU potente. Probablemente debas ejecutar en CPU.")

    if total_ram_gb < 8:
        sugerencias.append("La RAM es limitada; considera modelos muy pequeóos o cuantización agresiva.")
    elif total_ram_gb < 16:
        sugerencias.append("Podrías manejar modelos 7B con librerías optimizadas y/o swap si es imprescindible.")
    else:
        sugerencias.append("Tienes suficiente RAM para modelos medianos o móltiples procesos.")
